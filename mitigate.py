...
class OodleWithBudgetAbiThunks(BaseOodleHelper):
    ...
    @classmethod
    def init_module(cls):
        ffixv_exe_filepath = os.path.join(SCRIPT_DIRECTORY, "ffxiv.exe")
        ffixv_dx11_exe_filepath = os.path.join(SCRIPT_DIRECTORY, "ffxiv_dx11.exe")
        if POINTER_SIZE == 4:
            if not os.path.exists(ffixv_exe_filepath):
                raise RuntimeError("Need ffxiv.exe in the same directory. "
                                   "Copy one from your local Windows/Mac installation.")

            cls._module = OodleModule(PeImage(pathlib.Path(ffixv_exe_filepath).read_bytes()))
        elif POINTER_SIZE == 8:
            if not os.path.exists(ffixv_dx11_exe_filepath):
                raise RuntimeError("Need ffxiv_dx11.exe in the same directory. "
                                   "Copy one from your local Windows/Mac installation.")

            cls._module = OodleModule(PeImage(pathlib.Path(ffixv_dx11_exe_filepath).read_bytes()))
